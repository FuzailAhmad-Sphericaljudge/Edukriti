...
export function LessonClassroom({ lesson }: { lesson: LessonPlanGenerationResponse }) {
  ...
  }, [lesson.plan.language, speechAvailable, stopSpeech]);
  ...
  const startClass = () => {
    setHasStarted(true);
    if (speechAvailable) speakText(segment.narration);
  };
  ...
  return <main className="min-h-screen bg-[#071126] text-white">
    ...
    <div className="mx-auto grid max-w-7xl gap-4 p-4 lg:grid-cols-[minmax(0,1fr)_330px] lg:p-6">
      <section className="relative overflow-hidden rounded-[28px] border border-white/10 bg-[#0d1c38] shadow-2xl">
        {!hasStarted && <div className="absolute inset-0 z-30 grid place-items-center bg-[#071126]/92 p-6 text-center backdrop-blur-md"><div className="max-w-md"><div className="mx-auto grid size-16 place-items-center rounded-full bg-gradient-to-br from-sky-400 to-indigo-500 shadow-[0_0_55px_rgb(56_189_248/30%)]"><CirclePlay className="size-8 fill-white" /></div><p className="mt-5 text-xs font-bold uppercase tracking-[0.18em] text-sky-300">Your AI teacher is ready</p><h1 className="mt-2 font-heading text-3xl font-bold">Start your personalized class</h1><p className="mt-3 text-sm leading-6 text-white/60">One click starts Aarohi’s voice, synchronized captions, and the first visual explanation.</p><Button size="lg" onClick={startClass} className="mt-6 h-12 rounded-full bg-white px-8 text-slate-950 hover:bg-sky-50"><CirclePlay className="fill-slate-950" /> Start teaching</Button>{!speechAvailable && <p className="mt-3 text-xs text-amber-200">Voice is unavailable in this browser, but the complete visual lesson and captions will start.</p>}</div></div>}
