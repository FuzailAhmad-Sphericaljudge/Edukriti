  ...
  return <main className="min-h-screen bg-[#f5f7fb] px-5 py-7 text-slate-900 sm:px-8">
    <div className="mx-auto max-w-3xl">
      ...
      <section className="mt-5 rounded-[28px] border bg-white p-6 shadow-[0_16px_55px_rgb(30_42_70/8%)] sm:p-8">
        ...
        <div className="mt-7 flex justify-end">{!result ? <Button onClick={submit} disabled={submitting} size="lg" className="rounded-xl">{submitting ? <Loader2 className="animate-spin" /> : <Check />} Check answer</Button> : isLast ? <Button onClick={finish} disabled={finishing} size="lg" className="rounded-xl bg-emerald-600 hover:bg-emerald-700">{finishing ? <Loader2 className="animate-spin" /> : <Sparkles />} Create my report</Button> : <Button onClick={() => { setQuestionIndex((index) => index + 1); setError(''); }} size="lg" className="rounded-xl">Next question <ArrowRight /></Button>}</div>
      </section>
    </div>
  </main>;
}
