...
export default async function RevisionPage({ params }: { params: Promise<{ lessonId: string }> }) {
  ...
  return <main className="min-h-screen bg-[#f5f7fb] px-5 py-8 text-foreground sm:px-8"><div className="mx-auto max-w-5xl"><a href={`/lessons/${lessonId}/classroom`} className="inline-flex items-center gap-2 text-sm font-semibold text-muted-foreground hover:text-foreground"><ArrowLeft className="size-4" /> Back to classroom</a><header className="mt-6 overflow-hidden rounded-[30px] bg-[#15223f] p-7 text-white shadow-xl sm:p-9"><div className="flex items-start gap-4"><div className="grid size-12 shrink-0 place-items-center rounded-2xl bg-amber-300 text-slate-950"><Sparkles className="size-5" /></div><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-sky-300">Revision studio</p><h1 className="mt-2 font-heading text-3xl font-bold tracking-[-0.04em] sm:text-4xl">{plan.title}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-white/65">Review concise notes, test your recall with flashcards, and follow a focused seven-day plan.</p></div></div><a href={`/lessons/${lessonId}/practice`} className="mt-6 inline-flex h-11 items-center gap-2 rounded-xl bg-amber-300 px-5 text-sm font-bold text-slate-950 transition hover:bg-amber-200"><Sparkles className="size-4" /> Start adaptive practice</a></header><RevisionStudio plan={plan} /></div></main>;
}
